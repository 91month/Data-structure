...
int CreateUDG(ALGraph &G){
	...
	int i , k;
	fstream file;
	file.open("F:\\topo.txt");//该文件内容根据程序读写过程自己输入
	if (!file) {
	...
	for(k = 0; k < G.arcnum;++k)
	{
		VerTexType v1 , v2;
		int i , j,w;
		file >> v1 >> v2>>w; //输入一条边依附的两个顶点
		i = LocateVex(G, v1); //确定 v1 在 G 中位置，即顶点在 G.vertices 中的序号
		j = LocateVex(G, v2); //确定 v2 在 G 中位置，即顶点在 G.vertices 中的序号
		ArcNode *p1 = new ArcNode; //生成一个新的边结点*p1
		p1->adjvex=j; //邻接点序号为 j
		p1->nextarc = G.vertices[i].firstarc; //p1 与后结点连接
		G.vertices[i].firstarc=p1; //将新结点*p1 插入顶点 vi 的边表头部
		p1->weight = w;
		ArcNode *p2 = new ArcNode;//生成一个新的边结点*p2
		//参见上面邻接表生成程序段，以下补充语句生成逆邻接表
	}
	file.close();
    return OK;
}//CreateUDG
